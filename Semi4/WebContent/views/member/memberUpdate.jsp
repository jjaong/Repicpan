					...
					<td>&nbsp;&nbsp;선호요리</td>
						...
						<% for(Category c : list) { %>
							<%if(c.getCategoryName().equals(favoriteFood)) {%>
								<label for="<%=c.getCategoryName()%>"><input type="radio" name="favoriteFood" 
								id="<%=c.getCategoryName()%>" value="<%=c.getCategoryNo()%>" style="vertical-align:-9px;" checked><%=c.getCategoryName()%></label>
							<%}else{ %>
								<label for="<%=c.getCategoryName()%>"><input type="radio" name="favoriteFood" 
								id="<%=c.getCategoryName()%>" value="<%=c.getCategoryNo()%>" style="vertical-align:-9px;"><%=c.getCategoryName()%></label>
							<%} %>
						<%} %>
					</td>
				</tr>
